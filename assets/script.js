$(document).ready(function(){
    ...
    $('.menu_open a,.ajax_a').bind("click",function(e){
        ...
        $.post("ajax/"+href,{ajax:1}).done(function(data){
            history.pushState({key:href}, 'titre', href);
            $('html,body').scrollTop(0);
...
function setDispo(c_id){
	$('.menu-icon').addClass("loading");
	$.post('ajax/saisie_dispo.php',{c_id:c_id, dispo: $('#dispo_'+c_id).is(':checked')},function(){$('.menu-icon').removeClass("loading");});
}
...
function modifMbName(id)
{
	...
	if(newname!=undefined){$('.menu-icon').addClass("loading");
...
function effMb(id){
	if (confirm("Confirmer l'action")){$.post('ajax/effmb.php',
		{
			id:id
		},function(){
			window.location.reload();
